...
namespace IsMatch.StoriesDownloader.Common
{
    public class BaseSpider
    {
        ...
        public virtual IHtmlDocument GetHtmlDocumet(string address, Encoding encoding = null)
        {
            ...
            {
                wc.Headers.Add(HttpRequestHeader.ContentType, _Setting.CharCode);
        ...
        public virtual void GetList(string selectors, Action<IEnumerable<IHtmlElement>> htmlCollection, Action<string> WriteLog = null)
        {
            var document = GetHtmlDocumet(_Setting.TxtIndexUrl);
            var ddList = document.Find(selectors);
        ...
        public virtual void GetDetail(string selectors, Action<IHtmlElement, string> htmlCollection, Action<string> WriteLog = null)
        {
                ...
                for (int i = 0; i < threadCount; i++)
                {
                    ...
                    tasks[i] = Task.Factory.StartNew(() =>
                    {
                        foreach (var detailUrl in data)
                        {
                            ...
                            var detailHtml = document.FindFirst(selectors);
